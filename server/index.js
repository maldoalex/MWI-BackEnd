...
app.post("/contactForms", async (req, res) => {
  try {
    ...
    const newContactForm = await pool.query(
      "INSERT INTO contactform (first_name, last_name, email, title, message) VALUES ($1, $2, $3, $4, $5) RETURNING *",
      [first_name, last_name, email, title, message]
    );
...
app.get("/contactForms", async (req, res) => {
  try {
    const allContactForms = await pool.query("SELECT * FROM contactform");
...
app.get("/contactForms/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const contactForm = await pool.query(
      "SELECT * FROM contactform WHERE contactform_id = $1",
      [id]
    );
...
app.get("/headingOne", async (req, res) => {
  try {
    const allHeadingOnes = await pool.query("SELECT * FROM headingone");
...
app.get("/headingTwo", async (req, res) => {
  try {
    const allHeadingTwos = await pool.query("SELECT * FROM headingtwo");
...
app.get("/paragraph", async (req, res) => {
  try {
    const allParagraphs = await pool.query("SELECT * FROM paragraph");
